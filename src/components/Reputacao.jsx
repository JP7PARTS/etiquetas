...
const STATUS_META = {
  ...
  aguardar: { label: 'Aguardar até data', bg: 'var(--color-warning-bg)', fg: 'var(--color-warning-fg)', solid: '#92400e' },
  resolvida: { label: 'Resolvida ✓', bg: 'var(--color-success-bg)', fg: 'var(--color-success-fg)', solid: '#065f46' },
  recusada: { label: 'Recusada', bg: 'var(--color-error-bg)', fg: 'var(--color-error-fg)', solid: '#991b1b' },
};
          ...
          <>
            ...
            <div style={styles.chipRow}>
              ...
              {STATUS_ORDER.filter(k => statusCount[k]).map(k => (
                <button key={k} onClick={() => toggleStatus(k)}
                  style={{ ...styles.chip, ...(statusFiltro.has(k) ? { background: STATUS_META[k].solid, color: '#fff', borderColor: STATUS_META[k].solid } : {}) }}>
            ...
            <div style={styles.chipRow}>
              ...
              <button onClick={() => setPreenchFiltro(f => f === 'falta' ? '' : 'falta')}
                style={{ ...styles.chip, ...(preenchFiltro === 'falta' ? { background: '#c05621', color: '#fff', borderColor: 'var(--color-warning-border)' } : {}) }}>
    ...
    <div style={{ ...styles.caseCard, borderLeft: `5px solid ${sm.fg}`, ...(saiu ? { opacity: 0.7 } : {}) }}>
      ...
      <div style={styles.caseHead} onClick={onToggle} title={expanded ? 'Recolher' : 'Expandir'}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {pronta
            ? <span style={{ ...styles.prenBadge, background: 'var(--color-success-bg)', color: 'var(--color-success-fg)' }} title="Tem análise e argumento — já dá pra reclamar">✅ pronta</span>
        ...
        {expanded && (
          <button onClick={(e) => { e.stopPropagation(); onExcluir(c.numero_venda); }} title="Remover caso"
            style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--color-error-fg)', fontSize: '14px' }}>🗑</button>
        )}
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', margin: '4px 0' }}>
            {tentativas.map(t => (
              <div key={t.id} style={styles.tent}>
                <span style={{ ...styles.canalBadge, background: t.canal === 'humano' ? 'var(--color-info-bg)' : 'var(--color-info-bg)', color: 'var(--color-info-fg)' }}>
                ...
                <button onClick={() => onDelTent(c.numero_venda, t.id)} title="Remover tentativa"
                  style={{ marginLeft: 'auto', border: 'none', background: 'none', cursor: 'pointer', color: 'var(--color-error-fg)', fontSize: '12px' }}>🗑</button>
              </div>
            ))}
          </div>
        )}
...
const styles = {
  ...
  copyArg: { padding: '2px 8px', fontSize: '11px', fontWeight: 700, border: '1px solid var(--border)', borderRadius: '6px', background: 'var(--color-card)', color: 'var(--color-info-fg)', cursor: 'pointer' },
  caseCard: { border: '1px solid var(--border)', borderRadius: '10px', padding: '14px 16px', background: 'var(--color-card)', boxShadow: '0 2px 6px rgba(0,0,0,0.07)' },
  caseHead: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '8px', cursor: 'pointer', userSelect: 'none' },
  vendaNum: { fontFamily: 'monospace', fontWeight: 700, color: 'var(--color-info-fg)', textDecoration: 'none', fontSize: '13.5px' },
  ...
  proto: { fontSize: '12px', color: 'var(--color-info-fg)', textDecoration: 'none', fontFamily: 'monospace' },
  addTent: { display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap', marginTop: '4px' },
  canalTgl: { padding: '5px 10px', border: 'none', background: 'var(--color-card)', cursor: 'pointer', fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)' },
